...
describe('compare segments', () => {
  describe('not collinear', () => {
    ...
    it('events order in sweep line', () => {
      const se1 = new SweepEvent(
        [0, 1],
        true,
        new SweepEvent([2, 1], false, null, false),
        false
      );
      const se2 = new SweepEvent(
        [-1, 0],
      ...
      const se3 = new SweepEvent(
        [0, 1],
        true,
        new SweepEvent([3, 4], false, null, false),
        false
      );
      const se4 = new SweepEvent(
        [-1, 0],
        true,
        new SweepEvent([3, 1], false, null, false),
        false
      );

      assert.equal(compareEvents(se1, se2), 1);
      assert.notOk(se2.isBelow(se1.point));
      assert.ok(se2.isAbove(se1.point));
      ...
      assert.notOk(se4.isAbove(se3.point));
    });
    ...
    it('first point is below', () => {
      ...
      assert.notOk(se1.isBelow(se2.point));
      assert.equal(compareSegments(se1, se2), 1, 'compare segments');
    });
  });
  ...
  it('collinear segments', () => {
    const se1 = new SweepEvent(
      [1, 1],
      true,
      new SweepEvent([5, 1], false, null, false),
      true
    );
    const se2 = new SweepEvent(
      [2, 1],
